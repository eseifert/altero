 ...
 * dates. Choosing a language should not silently move somebody's date format to
 * another country's.
 */
export const useLocaleStore = defineStore('locale', () => {
   ...
   * in force, so its region survives; otherwise the bare language.
   */
  const formatting = computed(() => {
    const preferred = browserLanguages.value.find(
      (tag) => tag.replace('_', '-').split('-')[0].toLowerCase() === active.value,
    )
